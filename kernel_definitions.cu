...
__global__ void if_inner_kernel(const int N, const int K, const int M, const float *W, const uint8_t *X, float *Z) {
    extern __shared__ float shared_data[];
    ...
    if ((dataID < K * M/2) && (weightsID < (N * K - K/2))) {
        ...
        if (readBinArray(X, dataID)) {
            shared_data[tID] += W[weightsID];
        }

        if (readBinArray(X, dataID + int(M*K/2))) {
            shared_data[tID] += W[weightsID + int(K/2)];
        }
    }
...
void cuda_if_inner(const int N, const int K, const int M, const float *W, const uint8_t *X, float *Z) {
    float *dev_Z, *dev_W;
    uint8_t *dev_X;
    ...
    // launch kernel
    if_inner_kernel<<<gridSize, blockSize, sharedMemSize>>>(N, K, M, dev_W, dev_X, dev_Z);
...
int main() {
    ...
    for (int j = 0; j < 100; j++) {
        cuda_if_inner(N, K, M, W, X_binary, Z_binary);
    }
